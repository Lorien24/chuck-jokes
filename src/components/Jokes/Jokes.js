    ...
    {jokes.value === undefined && isCliked ? jokes.map(joke => 
        <div className="joke" key={joke.id} >
          ...
          <div  className="joke_footer">
            <p className="joke_update">Last update: {handleDate(joke.updated_at)} hours ago</p>
            {joke.categories[0] ? <p className="joke_category">{joke.categories[0]} </p> : null}
          </div>
        </div>
      ) : isCliked ? 
      <div className="joke">
        ...
        <div className="joke_footer">
          <p className="joke_update">Last update: {handleDate(jokes.updated_at)} hours ago</p>
          {jokes.categories[0] ? <p className="joke_category">{jokes.categories[0]} </p> : null}
        </div>
      </div>
     : null}
  </div>
